...
const StadiumSeat = () => {
  ...
  const handleSeatSelect = async (selectedObject) => {
    ...
    const isSeatReserved = async (label) => {
      try {
        const response = await axios.get(`http://localhost:5163/api/Tiketa/Exists/${label}`);
    ...
    try {
      ...
      if (isReserved) {
        ...
        setSelectedSeats(prevSelectedSeats => {
          if (!prevSelectedSeats.includes(label)) {
            return [...prevSelectedSeats, label];
          }
  ...
  const handleSubmit = async () => {
    console.log('Ulsat e zgjedhura:', selectedSeats);
    ...
    try {
      for (const ulsaEmri of selectedSeats) {
        ...
        // console.log(`Ulsa "${ulsaEmri}" u ruajt me sukses në server.`);
        setModalIsOpen(true);
      }
    } catch (error) {
  ...
  return (
    <div style={{ height: '600px' }}>
      ...
      <SeatsioSeatingChart
        ...
        region="eu"
        selectedObjects={selectedSeats.map(label => ({ label }))}
